...
def cargar_datos():
    ...
    return tipo_vehiculo, antiguedad, departamentos, meses, dias, anual, modelos


tipo_vehiculo, antiguedad, departamentos, meses, dias, anual, modelos = cargar_datos()

# ─────────────────────────────────────────────────────────────
# SIDEBAR  — nombres usados EXACTAMENTE en los elif
# ─────────────────────────────────────────────────────────────
SECCIONES = [
    "Inicio",
...
with st.sidebar:
    ...
    st.markdown("---")
    seccion = st.radio("Navegar a:", SECCIONES)
    st.markdown("---")
...
if seccion == "Inicio":
    ...
    st.markdown('<div class="sub-title">Técnicas de Machine Learning y Deep Learning aplicadas al RUNT · 2022–2025</div>', unsafe_allow_html=True)
    ...
    with col_a:
        ...
        sin un análisis sistemático profundo es imposible identificar qué características hacen
        ...
        st.info("Detectar los patrones y características de los vehículos que pueden causar un accidente de mayor severidad.")
    ...
    with col_b:
        st.markdown('<div class="section-header"> Desbalanceo de Clases</div>', unsafe_allow_html=True)
    ...
    with c2:
        ...
        afectando la detección de eventos críticos (como accidentes mortales).
        </div>""", unsafe_allow_html=True)
    with c3:
        st.markdown("""<div class="hallazgo-card">
        <b>Mannering et al. (2016)</b><br>
        La predicción de severidad permite anticipar escenarios de riesgo
...
# ─────────────────────────────────────────────────────────────
    ...
    with tab1:
        ...
        with col_izq:
            ...
            fig.add_vline(x=5.0, line_dash="dash", line_color="red",
                          annotation_text="Promedio global (5%)")
            fig.update_layout(title="Tasa de mortalidad (%) por tipo de vehículo",
        ...
        with col_der:
            ...
            hallazgos = [
                (" Carga pesada lidera la mortalidad",
                 "Tractocamión, volqueta y camión registran tasas 3 a 6 veces el promedio global (aproximadamente el 5%)."),
                (" Volumen vs. letalidad",
                 "Las motocicletas tienen el mayor volumen de accidentes, pero su tasa es cercana al promedio."),
                ...
                (" V de Cramér",
                 "El tipo de vehículo tiene una asociación moderada con la gravedad, mientras que la marca tiene una asociación débil."),
            ]
        ...
        fig2 = px.scatter(
            tipo_vehiculo, x="Volumen", y="Tasa_Mortalidad",
    ...
    with tab2:
        ...
        fig3.add_vline(x=5.0, line_dash="dash", line_color="red",
                       annotation_text="Promedio global")
        fig3.update_layout(title="Tasa de mortalidad por marca (Top 10)",
...
# ─────────────────────────────────────────────────────────────
    ...
    with col1:
        ...
        fig.add_hline(y=5.0, line_dash="dash", line_color="red",
                      annotation_text="Promedio global (5%)")
        fig.update_layout(
            title="Tasa de mortalidad por antigüedad del vehículo<br><sup>Con intervalos de confianza Wilson 95%</sup>",
    ...
    with col2:
        ...
        hallazgos2 = [
            (" Gradiente de riesgo monótono",
             "A mayor antigüedad del vehículo, mayor tasa de mortalidad. Relación clara y consistente."),
            (" Vehículos con edad mayor a 20 años tienen máximo riesgo",
             "Tasa de 9.4% — casi el doble del promedio. Sus IC no se solapan con los de vehículos nuevos."),
        ...
        for titulo, desc in hallazgos2:
            st.markdown(f"""<div class="hallazgo-card">
            <b>{titulo}</b><br><small>{desc}</small>
            </div>""", unsafe_allow_html=True)

        st.markdown("---")
        st.markdown("** Prueba Mann-Whitney U**")
        st.code("p = 1.13e-65\n→ Diferencia significativa")
        st.markdown("---")
        st.markdown("**Estadística por gravedad**")
...
# ─────────────────────────────────────────────────────────────
    ...
    tab_geo, tab_tiempo = st.tabs([" Geografía", " Temporalidad"])
    ...
    with tab_geo:
        ...
        with col2:
            ...
            <b>periféricos</b> concentran la mayor mortalidad por déficit hospitalario.
            </div>
            <div class="hallazgo-card">
            <b> Subregistro en periferias</b><br>
            Los accidentes leves en municipios remotos tienen menor tasa de reporte,
            lo que puede aumentar las tasas observadas en esas regiones.
            </div>
            """, unsafe_allow_html=True)
    ...
    with tab_tiempo:
        ...
        with col_a:
            ...
            fig_mes.update_layout(title="Accidentalidad y mortalidad mensual",
        ...
        with col_b:
            colores_dias = ["#065A82" if d in ["Viernes", "Sábado", "Domingo"]
                            else "#AED6EF" for d in dias["Dia"]]
            fig_dia = make_subplots(specs=[[{"secondary_y": True}]])
            fig_dia.add_trace(
                go.Bar(x=dias["Dia"], y=dias["Accidentes"],
                       name="Accidentes", marker_color=colores_dias),
                secondary_y=False
            )
            fig_dia.add_trace(
                go.Scatter(x=dias["Dia"], y=dias["Tasa_Mortalidad"],
                           name="Tasa mortalidad (%)", mode="lines+markers",
                           marker_color="#E63946", line_width=2),
                secondary_y=True
            )
            fig_dia.update_layout(
                title="Accidentalidad por día<br><sup>Azul oscuro = fin de semana</sup>",
                height=380, legend=dict(orientation="h", y=-0.2)
            )
        ...
        col_c1.success("** Julio: mayor volumen**\n\nal ser vacaciones de mitad de año concentran el mayor número de accidentes.")
        col_c2.error("** Anomalía julio 2024**\n\n22,667 registros inválidos de la Secretaría de Medellín fueron excluidos por errores de digitación.")
...
# ─────────────────────────────────────────────────────────────
    ...
    st.markdown('<div class="main-title"> Modelos de Machine Learning y Deep Learning</div>', unsafe_allow_html=True)
    ...
    with tab_rank:
        ...
        with col2:
            ...
            <b> Por qué no usamos Accuracy</b><br>
            Un modelo que prediga siempre "Con Heridos" tendría 95% de accuracy
            pero detectaría 0% de muertes. El F1-Macro penaliza estas muertes por igual los
    ...
    with tab_metricas:
        ...
        st.markdown("#### Comparación multidimensional")
    ...
    with tab_pipeline:
        ...
        pasos = [
            ...
            ("2. Partición estratificada",
             ["80% entrenamiento — 20% prueba", "stratify=y → preserva proporción 95/5 del desbalanceo",
              "Anti-leakage: features post-split calculadas SOLO sobre y_train"]),
            ("3. Preprocesador",
             ["Numéricas (8): imputación mediana + StandardScaler",
              "Categóricas (2): imputación moda + OrdinalEncoder",
              "Se descartó OHE por explosión de dimensiones (aproximadamente 2,914 columnas)"]),
            ("4. Entrenamiento",
...
# ─────────────────────────────────────────────────────────────
    ...
    conclusiones = [
        ("", "Predictibilidad estadística",
         "La severidad de los accidentes en Colombia es estadísticamente predecible a partir de variables vehiculares, geográficas y temporales."),
        ("", "Tipo de vehículo: predictor más fuerte",
         "Los vehículos de carga pesada registran tasas de mortalidad mayores al promedio. Las motocicletas tienen alto volumen pero tasa cercana al promedio."),
        ("", "Antigüedad amplifica la mortalidad",
         "Vehículos con más de 20 años tienen el doble del riesgo que vehículos nuevos. Los sistemas de seguridad pasiva (airbags, ABS) son determinantes."),
        ("", "Paradoja geográfica volumen/letalidad",
         "Los departamentos con más accidentes tienen tasas más bajas. Los periféricos concentran la mayor mortalidad por déficit hospitalario."),
        ("", "Patrones temporales",
         "Julio, diciembre y fines de semana concentran los picos."),
    ...
    for i in range(0, len(conclusiones), 2):
        ...
        for j, col in enumerate(cols):
            if i + j < len(conclusiones):
                ...
                border-radius:10px;padding:1.2rem;margin-bottom:0.8rem;min-height:140px">
